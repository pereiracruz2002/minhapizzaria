 ...
 */
function vw_restaurant_lite_logo_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control( 'logo_size', array(
		'label'       => esc_html__( 'Logo Size','vw-restaurant-lite' ),
...
add_action( 'customize_register', 'vw_restaurant_lite_logo_customize_register' );

/**
 * Add support for logo resizing by filtering `get_custom_logo`
 */
function vw_restaurant_lite_customize_logo_resize( $html ) {
	...
	if ( is_numeric( $size ) && is_numeric( $custom_logo_id ) ) {
		...
		if ( $logo['width'] >= $logo['height'] ) {
			$output = vw_restaurant_lite_min_max( $logo['height'], $logo['width'], $max['height'], $max['width'], $size, $min );
			$img = array(
				'height'	=> $output['short'],
				'width'		=> $output['long']
			);
		// portrait
			...
			$output = vw_restaurant_lite_min_max( $logo['width'], $logo['height'], $max['width'], $max['height'], $size, $min );
			$img = array(
				'height'	=> $output['long'],
				'width'		=> $output['short']
			);
...
add_filter( 'get_custom_logo', 'vw_restaurant_lite_customize_logo_resize' );

/* Helper function to determine the max size of the logo */
function vw_restaurant_lite_min_max( $short, $long, $short_max, $long_max, $percent, $min ){
	$ratio = ( $long / $short );
...
function vw_restaurant_lite_customize_preview_js() {
	wp_enqueue_script( 'vw-restaurant-lite-customizer', get_template_directory_uri() . '/js/logo/customize-preview.js', array( 'jquery', 'customize-preview' ), '201709081119', true );
}
 ...
 */
function vw_restaurant_lite_customize_controls_js() {
		wp_enqueue_script( 'vw-restaurant-lite-customizer-controls', get_template_directory_uri() . '/js/logo/customize-controls.js', array( 'jquery', 'customize-preview' ), '201709071000', true );
}
add_action( 'customize_controls_enqueue_scripts', 'vw_restaurant_lite_customize_controls_js' );

/**
 * Adds CSS to the Customizer controls.
 */
function vw_restaurant_lite_customize_css() {
	wp_add_inline_style( 'customize-controls', '#customize-control-logo_size input[type=range] { width: 100%; }' );
}
add_action( 'customize_controls_enqueue_scripts', 'vw_restaurant_lite_customize_css' );

/**
 * Testing function to remove logo_size theme mod
 */
function vw_restaurant_lite_remove_theme_mod() {
	if ( isset( $_GET['remove_logo_size'] ) && 'true' == $_GET['remove_logo_size'] ){
		set_theme_mod( 'logo_size', '' );
	}
}
add_action( 'wp_loaded', 'vw_restaurant_lite_remove_theme_mod' );
