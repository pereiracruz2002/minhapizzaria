	...
	function process_orderstatus_change($blog_id, $order_id, $order_status){
			...
			**********************************************/
			...
			if(!empty($order['orders'])){
				...
				*****************************************/
				$order_formatted = reset($order['orders']);
				/****************************************
				 	get blog options
				*****************************************/
				$blog_options = $order_formatted['blog_options'];
					...
					*/
					if(!empty($template_options[$template_id])){
						...
						*/
						...
						$order_sections['sections'] = $order_formatted;//temp workaround, until all  instances of get_template_email_plaintext_sections_markup get ficxed/streamlined


						/* always get plaintext */
						$template_parameters = WPPIZZA() -> templates_email_print -> get_template_email_plaintext_sections_markup($order_sections, $blog_options['localization'], $template_values, $template_type );
						$template_markup_plaintext = $template_parameters['markup'];

						/* add html if set */
						if($template_values['mail_type'] == 'phpmailer'){
							$template_markup_html = WPPIZZA() -> templates_email_print -> get_template_email_html_sections_markup($order_sections, $blog_options['localization'], $template_values, $template_type, $template_id);
						}
					}

				}
				/****************************************
				 	for consistancy with old versions
				 	remove blog options from main order formatted
				 	array and only keep it in $blog_options var
				*****************************************/
				unset($order_formatted['blog_options']);
