...
class WPPIZZA_DASHBOARD_WIDGETS{
	...
	*
	*********************************************************/
	function wppizza_do_dashboard_widget_sales() {


		/***get sales data returns error if <5.3***/
		$REPORTS = new WPPIZZA_REPORTS;
		$data = $REPORTS  -> wppizza_report_dataset(false, WPPIZZA_ADMIN_DASHBOARD_TRANSIENT_REPORTS_EXPIRY, true);/*export false, transients true, dahsboard true*/
		...
		$dpwDashicon='<span class="wppizza-dashicons-medium dashicons-update wppizza-dashboard-widget-update" title="'.__('update now', 'wppizza-admin').'"></span>';
		echo'<div class="wppizza-dash wppizza-dash-updated">'.__('last update ','wppizza-admin').' '.date('Y-m-d H:i:s',$transientSetAt).' '.$dpwDashicon.'</div>';
