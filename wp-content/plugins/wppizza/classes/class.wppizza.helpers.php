...
class WPPIZZA_HELPERS{
	...
	***************************************/
	function enabled_formfields($keys_only = false, $enabled_only = true, $omit_tips = false, $page = null){  
		global $wppizza_options;
		...
		*/
		...
		/* allow filtering */
		$wppizza_options['order_form'] = apply_filters('wppizza_filter_formfields', $wppizza_options['order_form'], $page);
		/* alias filter - going forward the above will - at some point - be deprecated to avoid confusion with other filters*/
		$wppizza_options['order_form'] = apply_filters('wppizza_register_formfields', $wppizza_options['order_form'], $page);
	...
	***************************************/
	function itemised_order_columns($order_parameters, $template_parameters, $template_type, $localization, $return_markup = true ){
			...
			$column_th[0] 	= array('key'=>'quantity', 'label'=>trim($localization['itemised_label_quantity']));
			$column_th[1] 	= array('key'=>'article', 'label'=>trim($localization['itemised_label_article']));
			$column_th[2] 	= array('key'=>'taxrate', 'label'=>trim($localization['itemised_label_taxrate']));
			$column_th[3] 	= array('key'=>'price', 'label'=>trim($localization['itemised_label_price']));
			...
			*/
			$columns = apply_filters('wppizza_filter_itemised_order_columns', $columns, $localization, $template_type);
			...
			if($return_markup){
				...
				*/
				...
				foreach($order_parameters['sections'][$section_key]['items'] as $itemKey => $arr){
					...
					/* filter items array */
					$item_count++;
					$rows[$itemKey] = apply_filters('wppizza_filter_templates_item_markup_plaintext', $rows[$itemKey], $itemKey , $arr, count($itemised_row), $order_parameters['sections'][$section_key]['items'], $item_count, $order_id, $localization, $template_type);
				}}
				...
				*/
				...
				foreach($order_parameters['sections'][$section_key]['items'] as $itemKey => $arr){
					...
					/* filter items array */
					$item_count++;
					$menu_item = apply_filters('wppizza_filter_templates_item_markup_html',$menu_item, $itemKey , $arr, $order_parameters['sections'][$section_key]['items'], $column_count, $item_count, $order_id, $localization, $template_type);
