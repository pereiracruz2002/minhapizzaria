			...
			if($ff['type'] != 'tips'){
				...
				*/				
				...
				if($ff['type']=='multicheckbox'){
					...
					//$formfields[$key]['field'] .= '---'.print_r($ff['value'],true);
					foreach($ff['options'] as $k => $option){
						$formfields[$key]['field'] .= '<label><input id="'. $key .'_'.$k.'" value="'. $option .'" name="'. $key.'[]"  type="checkbox" value="'.$k.'"  ' . $ff['required_attribute'] . ' '.checked(!empty($ff['value']) && in_array($option, $ff['value']),true, false).'/>'.$option.' </label>';
					}
