	...
	****************************************************************/
	...
	function set_default_templates(){
		...
		foreach($template_types as $template_type => $template_ident){
			$template_options[$template_ident][0] = WPPIZZA()->order->results_formatted($template_ident, false, false, false, false, $template_type, 0, false, true );
		}
		...
		************************************/
			...
			echo"<div id='".WPPIZZA_SLUG."_list_".$this->class_key."_custom' class='".WPPIZZA_SLUG."_list_".$this->class_key."'>";
				...
				foreach($pagination['list'] as $template_key=>$template_values){
					$tpl = WPPIZZA()->templates_email_print->admin_template($template_key, $template_type, $template_values);//, $template_values
					print"". $tpl ."";
				}
