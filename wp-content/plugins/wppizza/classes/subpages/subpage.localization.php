...
class WPPIZZA_LOCALIZATION{
	...
	*********************************************************/
	public function wppizza_admin_settings_section_fields($args){
		...
		/**is textarea**/
		$is_textarea= !empty($args['textarea']) ? true : false ;
		/**is plaintext textarea**/
		$is_textarea_plaintext = !empty($args['textarea_plaintext']) ? true : false ;		
			...
			echo '<label>';
				...
				*/
				if($is_textarea_plaintext){
					echo "<textarea name='".WPPIZZA_SLUG."[".$options_key."][".$field."]' cols='51' rows='5'>".esc_html($wppizza_options[$options_key][$field])."</textarea>";	
				}else{
					echo "<input name='".WPPIZZA_SLUG."[".$options_key."][".$field."]' size='50' type='text' value='".esc_html($wppizza_options[$options_key][$field])."'>";				
				}
		...
		/********************************
		*	[add action for modules to hook into]
		********************************/
//		do_action('wppizza_admin_settings_section_fields_'.$this->class_key.'', $wppizza_options, $options_key, $field, $label, $description);
	}
	...
	****************************************************************/
	function options_validate($options, $input){
		...
		if(isset($_POST[''.WPPIZZA_SLUG.'_'.$this->class_key.''])){
			...
			$allowHtml=array();
			$allowHtml = apply_filters('wppizza_filter_localization_html', $allowHtml);
