	...
	function sort_and_calculate_cart($is_checkout = null, $recalculate_all = false){
		...
		*********************************************************/
		...
		if(!empty($wppizza_options['order_settings']['discount_calculate_delivery_before_discount'])){
			$this_order_total_price_items_for_delivery -=  $this_order_discount;
		}
		...
		if(!$this_order_tax_included){
			...
			*/
			if(!empty($this_order_discount)){
				/** combined tax **/
				$this_order_taxes_to_add -= $this_order_discount / 100 * $this_order_taxrates_average_per_item;

				/** tax by rate **/
				foreach($this_order_taxes_at_rate as $tbrKey => $txbyrate){
					$this_order_taxes_at_rate[$tbrKey]['to_add'] -=  ($this_order_discount * $txbyrate['item_at_taxrate']) / 100 * $txbyrate['rate'] ;
				}
			}
		...
		**********************************************************/
		if($this_order_tax_included){
			...
			*/
			if(!empty($this_order_discount)){
				/** combined tax **/
				$this_order_taxes_included -= $this_order_discount / (100 + $this_order_taxrates_average_per_item) * $this_order_taxrates_average_per_item;
				/** tax by rate **/
				foreach($this_order_taxes_at_rate as $tbrKey => $txbyrate){
					$this_order_taxes_at_rate[$tbrKey]['included'] -=  ($this_order_discount * $txbyrate['item_at_taxrate']) / (100 + $this_order_taxrates_average_per_item) * $txbyrate['rate'];
				}
			}
		...
		***************************************/
		$totalOrderBeforeTipsAndHandling = $this_order_total_price_items - $this_order_discount + $this_order_delivery_charges + $this_order_taxes_to_add;
		...
		***************************************/
		$total = $total_price_items - $discount + $delivery_charges + $taxes_to_add + $handling_charges + $tips;
