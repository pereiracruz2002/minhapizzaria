...
class WPPIZZA_EMAIL{
	...
	*************************************************/
	...
	/*************************************************
		wrapper fucntion
	*************************************************/
	function send($emails_to_send, $order_formatted, $order_details_raw){
		/*
			setup phpmailer and send emails one by one as required
		*/
		$results = $this->send_emails($emails_to_send, $order_formatted, $order_details_raw);
	return $results;
	}
	...
	*************************************************/
	function send_emails($emails_to_send, $order_formatted, $order_details_raw){
		...
		********************************************************************************************/
		foreach($emails_to_send as $recipient_key => $email_settings){
			...
			*************************************************/
			...
			/* allow filtering */
			$wp_mail_headers = apply_filters('wppizza_filter_mail_headers', $wp_mail_headers, $recipient_key, $email_settings, $order_formatted, $order_details_raw);
			...
			*************************************************/
			...
			/* allow filtering */
			$wp_mail_attachments = apply_filters('wppizza_filter_mail_attachments', $wp_mail_attachments, $recipient_key, $email_settings, $order_formatted, $order_details_raw);
			...
			*************************************************/
			...
			if(!empty($wppizza_options['tools']['disable_emails'])){
				...
				if(wp_mail($wp_mail_recipients, $email_subject, $message, $wp_mail_headers, $wp_mail_attachments)) {
					...
					do_action('wppizza_on_email_sent', $recipient_key, $email_settings, $order_formatted, $order_details_raw);
				}else{
	...
	function set_html_mail_body($phpmailer){
		...
		*/
		...
		if( $phpmailer->ContentType == 'text/html' ) {
			...
			if(!empty($this->message_body['plaintext'])){
				$phpmailer->Body = $this->message_body['plaintext'];
				$phpmailer->AltBody = '';
			}
