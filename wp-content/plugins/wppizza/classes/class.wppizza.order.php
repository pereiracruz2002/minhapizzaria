...
class WPPIZZA_ORDER{
	...
	public static function session_formatted($called_by = 'session'){
		global $wppizza_options, $blog_id;
		static $test=0, $order_details_formatted = null;
		$test++;
		...
		if($order_details_formatted == null){
			...
			*/
			...
			/* blog_options  */
			$order_details_formatted['blog_options'] = self::blog_options($called_by, $order_values);

			/* customer confirmation form inputs - formatted */
			$order_details_formatted['confirmation'] = self::customer_confirmation_formatted($called_by);

			/* order siteinfo - formatted */
			$order_details_formatted['site'] = self::site_details_formatted($called_by, $order_values);

			/* general order details - formatted */
			$order_details_formatted['ordervars'] = self::general_ordervars_formatted($called_by, $order_values);

			/* customer details - formatted */
			$order_details_formatted['customer'] = self::customer_details_formatted($called_by, $order_values);

			/* order itemised - formatted */
			$order_details_formatted['order'] = self::itemised_details_formatted($called_by, $order_values);

			/* order summary - formatted */
			$order_details_formatted['summary'] = self::summary_details_formatted($called_by, $order_values);
	...
	/*
		passing on full order variables
		OR
		session = true
		OR
		order_id (with optional blog id)

	*/
	function results_formatted($called_by, $order = false, $session = false , $order_id = false, $blog_id = false, $template_type = false, $template_key = false, $set_values = false, $install = false, $order_formatted = false){

		global $wppizza_options, $blog_id;
		/* localized vars do note exist on install */
		$txt = !empty($install) ? $wppizza_options['localization'] + $wppizza_options['confirmation_form']['localization'] : array();


		$order_details_formatted = array();
		$get_template_parameters = false;

		/** only get labels (templates admin) */
		if(!$order && !$session && !$order_id){
			global $wppizza_options;
			$order_values = array();
			$order_values['blog_options'] = $wppizza_options;
			$get_template_parameters = $template_type;
		}
		/** use parameters passed on */
		if($order && !$session && !$order_id){
			$order_values = $order;
		}
		/** select from a distinct completed order by order_id */
		if(!$order && !$session && !empty($order_id)){
			$order_values = WPPIZZA()->db->fetch_order_details($blog_id, $order_id, false, 'COMPLETED');
			/** order details are always arrays */
			$order_values = $order_values['orders'][0];
		}

		/** use/add parameters to order from current user session data  */
		if($order && $session){
			$order_values = $order;
			$user_session	= WPPIZZA()->session->get_userdata();
			/* get checkout parameters (can_checkout, is_checkout, min order etc ) order page*/
			$checkout_parameters	= WPPIZZA()->db->map_order($user_session, true);
			foreach($user_session as $usKey=>$usVal){
				if(!isset($order_values['customer_ini'][$usKey])){
					$order_values['customer_ini'][$usKey] = maybe_unserialize($usVal);
				}
			}
		}
		...
		*/
		...
		/*
			order of keys here will also determine the default
			order of sections for templates
		*/
		/* global settings  */
		$order_details_formatted = self::global_parameters($called_by, $order_values, $get_template_parameters, $template_key, $set_values, $order_formatted, $txt);

		/* global styles  */
		$order_details_formatted['global_styles'] = self::global_styles($called_by, $order_values, $get_template_parameters, $template_key, $set_values, $order_formatted, $txt);
		...
		/* blog_options  */
		$order_details_formatted['blog_options'] = self::blog_options($called_by, $order_values, $get_template_parameters);

		/* customer confirmation form inputs - formatted */
		$order_details_formatted['confirmation'] = self::customer_confirmation_formatted($called_by, $get_template_parameters);

		/* order siteinfo - formatted */
		$section_parameters['site'] = self::site_details_formatted($called_by, $order_values, $get_template_parameters, $template_key, $set_values, $order_formatted);

		/* general order details - formatted */
		$section_parameters['ordervars'] = self::general_ordervars_formatted($called_by, $order_values, $get_template_parameters, $template_key, $set_values, $order_formatted);

		/* customer details - formatted */
		$section_parameters['customer'] = self::customer_details_formatted($called_by, $order_values, $get_template_parameters, $template_key, $set_values, $order_formatted, $session);


		/* order itemised - formatted */
		$section_parameters['order'] = self::itemised_details_formatted($called_by, $order_values, $get_template_parameters, $template_key, $set_values, $order_formatted);

		/* order summary - formatted */
		$section_parameters['summary'] = self::summary_details_formatted($called_by, $order_values, $get_template_parameters, $template_key, $set_values, $order_formatted);


		/*
			default section sort order
		*/
		$section_sort = array('site', 'ordervars', 'customer', 'order', 'summary');

		/*
			reorder to set sort order if getting values from saved template variables
		*/
		if(!empty($set_values)){
			$section_sort = array();
			foreach($set_values['sort'] as $sort_section_key => $sort_section_values){
				$section_sort[] = $sort_section_key;
			}
		}
		/* get section variables in required sort order */
		foreach($section_sort as $section_key){
			$order_details_formatted['sections'][$section_key] = $section_parameters[$section_key];
		}

		/*
		 installing plugin, add first template, only save relevant vars
		*/
		if($install){
			...
			/* temp var */
			$install_values = $order_details_formatted;


			$order_details_formatted = array();

			$order_details_formatted['sort'] = array();
			$order_details_formatted['title'] = $install_values['title'];
			$order_details_formatted['mail_type'] = $install_values['mail_type'];
			$order_details_formatted['omit_attachments'] = !empty($install_values['omit_attachments']) ? $install_values['omit_attachments'] : false;
			$order_details_formatted['recipients_additional'] = !empty($install_values['recipients_additional']) ? $install_values['recipients_additional'] : array();
			$order_details_formatted['global_styles'] = $install_values['global_styles'];

			$order_details_formatted['sections'] = array();
			foreach($install_values['sections'] as $section_key=>$section_values){
				$order_details_formatted['sort'][$section_key] = array();

				$order_details_formatted['sections'][$section_key] = array();

				$order_details_formatted['sections'][$section_key]['style'] = !empty($section_values['style']) ? $section_values['style'] : array();

				if(!empty($section_values['section_enabled'])){
					$order_details_formatted['sections'][$section_key]['section_enabled'] = true;
				}

				if(!empty($section_values['label_enabled'])){
					$order_details_formatted['sections'][$section_key]['label_enabled'] = true;
				}
				$order_details_formatted['sections'][$section_key]['parameters'] = array();


				if(!empty($section_values['parameters'])){
				foreach($section_values['parameters'] as $section_paramater_key=>$section_paramater_value){
					$order_details_formatted['sort'][$section_key][$section_paramater_key] = true;

					if(!empty($section_paramater_value['enabled'])){
						$order_details_formatted['sections'][$section_key]['parameters'][$section_paramater_key]['enabled'] = true;
					}
				}}
			}
		}
	...
	/*
		allow filtering - typically one would probably only need the first 2 or perhaps 3 parameters, but lets add complete order_values too. maybe someone needs it ,
		skip if just getting template parameters, as there will be no order values
	...
	******************************************************************/
	function simplify_order_values($order_results, $blog_options = false, $class_idents = false){
		...
		****************************************/
		...
		/* unset tips as they are available in summary */
...
public static function global_parameters($called_by, $order = false, $get_template_parameters = false, $template_key = 1, $set_values = false, $order_formatted = false, $txt = false){
	global $wppizza_options;
	...
	$global_parameters = array();
			...
			*/
			if($get_template_parameters=='emails'){
				...
				*/
				//$template_parameters['recipients_default_selected'] = (empty($set_values)) ? array()  : $wppizza_options['templates_apply'][$get_template_parameters]['recipients_default'] ;
				$template_parameters['recipients_default_selected'] = $wppizza_options['templates_apply'][$get_template_parameters]['recipients_default'] ;
				/*
					additional recipients
				*/
				$template_parameters['recipients_additional']= (empty($set_values['recipients_additional'])) ? '' : implode(',',$set_values['recipients_additional']) ;
				/*
					omit_attachments
				*/
				$template_parameters['omit_attachments'] = (empty($set_values['omit_attachments']) ) ? false  : true ;
			}
			...
			*/
			if($get_template_parameters=='print'){
				/*
					is this the print template in use ?
				*/
				$template_parameters['print_id'] = (empty($set_values)) ? false  : ( $wppizza_options['templates_apply'][$get_template_parameters] == $template_key ? true : false) ;
			...
			*/
			if($get_template_parameters=='print' || $get_template_parameters=='emails'){

				/*
					class to identify we are adding a new template
				*/
				$template_parameters['new_class'] = (empty($set_values)) ? ''.WPPIZZA_SLUG.'-'.$templates_ident.'-new' : '' ;
				/*
					admin sort order
				*/
				//$template_parameters['admin_sort'] = (empty($set_values)) ? '' : $set_values['admin_sort'] ;
				/*
					title - if ID ==0 title == default (on install)
				*/
				$template_parameters['title'] = (empty($set_values)) ? ( ($template_key == 0) ? __('default', 'wppizza-admin').'' : __('new', 'wppizza-admin').' [ID:'.$template_key.'] ') : $set_values['title'] ;
				/*
					mail/display type, icon classes
				*/
				$template_parameters['mail_type'] = (empty($set_values) ) ? 'wp_mail'  : $set_values['mail_type'] ;
				/* icon active/inactive class */
				$template_parameters['htmlactiveclass'] = ($template_parameters['mail_type']=='phpmailer') ? ''.WPPIZZA_SLUG.'_'.$templates_ident.'_style_toggle '.WPPIZZA_SLUG.'-dashicons-'.$templates_ident.'-'.$get_template_parameters.'-media-code' : ' '.WPPIZZA_SLUG.'-dashicons-'.$templates_ident.'-'.$get_template_parameters.'-media-code-inactive' ;
			...
			if(!empty($get_template_parameters)){
...
*
********************************************/
public static function global_styles($called_by, $order = false, $get_template_parameters = false, $template_key = false, $set_values = false, $order_formatted = false, $txt = false){

	$section_key = 'global_styles';

	$global_styles = array();/* currently an empty array */
		...
		if($get_template_parameters){
			...
			*************************************************************************/
			if(!empty($set_values)){
				...
				$template_parameters['body'] = !empty($set_values[$section_key]['body']) ? $set_values[$section_key]['body'] : '';

				if($get_template_parameters=='emails'){
					$template_parameters['wrapper'] = !empty($set_values[$section_key]['wrapper']) ? $set_values[$section_key]['wrapper'] : '';
					$template_parameters['table'] = !empty($set_values[$section_key]['table']) ? $set_values[$section_key]['table'] : '';
				}
			}
...
********************************************/
public static function blog_options($called_by, $order = false, $get_template_parameters = false){
	$blog_options = array();
...
*
********************************************/
public static function site_details_formatted($called_by, $order = false, $get_template_parameters = false, $template_key = false, $set_values = false, $order_formatted = false){
	...
	$site_parameters = array();
		...
		*/
		...
		/*
			allow filtering
		*/
		$site_parameters =apply_filters('wppizza_filter_site_details_formatted', $site_parameters , $order, $get_template_parameters);
		...
		if($get_template_parameters){
			...
			*/
			...
			/*
				label for section
			*/
			$template_enabled_parameters['labels']['label'] 	= $order['blog_options']['localization']['templates_label_'.$section_key.''];
			/*
			...
			*************************************************************************/
			if(!empty($set_values)){
				/* global section styles */
				if($get_template_parameters=='emails'){
            		$template_enabled_parameters['style']['table'] = !empty($set_values['sections'][$section_key]['style']['table']) ? $set_values['sections'][$section_key]['style']['table'] : '';
            		$template_enabled_parameters['style']['th'] = !empty($set_values['sections'][$section_key]['style']['th']) ? $set_values['sections'][$section_key]['style']['th'] : '';
            		$template_enabled_parameters['style']['td-ctr'] = !empty($set_values['sections'][$section_key]['style']['td-ctr']) ? $set_values['sections'][$section_key]['style']['td-ctr'] : '';
				}

				/* section enabled */
				$template_enabled_parameters['section_enabled'] = !empty($set_values['sections'][$section_key]['section_enabled']) ? true : false;
				/* section label enabled */
				$template_enabled_parameters['label_enabled'] = !empty($set_values['sections'][$section_key]['label_enabled']) ? true : false;

				$resort = 0;
				foreach($set_values['sort'][$section_key] as $parameter_key=>$enabled){
					/* resort */
					$template_enabled_parameters['parameters'][$parameter_key]['sort'] = $resort;
					/* enabled ?*/
					$template_enabled_parameters['parameters'][$parameter_key]['enabled'] = !empty($set_values['sections'][$section_key]['parameters'][$parameter_key]['enabled']) ? true : false;
					/* css*/
					if($get_template_parameters=='emails'){
					$template_enabled_parameters['style'][$parameter_key.'-tdall'] = !empty($set_values['sections'][$section_key]['style'][$parameter_key.'-tdall']) ? $set_values['sections'][$section_key]['style'][$parameter_key.'-tdall'] : '';
					}
...
*
********************************************/
public static function general_ordervars_formatted($called_by, $order = false, $get_template_parameters = false, $template_key = false, $set_values = false, $order_formatted = false){

		$section_key = 'ordervars';

		/*
			only get labels/keys
		*/
		if($get_template_parameters){
			$blog_options = $order['blog_options'];
		}
		...
		*/
		if($get_template_parameters){
			...
			$order_parameters['payment_method']['value'] = ($gw_settings->gateway_type == 'cod') ? $blog_options['localization']['common_value_order_cash'] : $blog_options['localization']['common_value_order_credit_card'] ;
		...
		*/
			...
			*************************************************************************/
			if(!empty($set_values)){
				...
				if($get_template_parameters=='emails'){
            		$template_enabled_parameters['style']['table'] = !empty($set_values['sections'][$section_key]['style']['table']) ? $set_values['sections'][$section_key]['style']['table'] : '';
            		$template_enabled_parameters['style']['th'] = !empty($set_values['sections'][$section_key]['style']['th']) ? $set_values['sections'][$section_key]['style']['th'] : '';
            		$template_enabled_parameters['style']['td-lft'] = !empty($set_values['sections'][$section_key]['style']['td-lft']) ? $set_values['sections'][$section_key]['style']['td-lft'] : '';
            		$template_enabled_parameters['style']['td-rgt'] = !empty($set_values['sections'][$section_key]['style']['td-rgt']) ? $set_values['sections'][$section_key]['style']['td-rgt'] : '';
				}
		...
		/*
			temporary fix (dirty hack really) for wppizza_crn == 1.0 as it wants an object not an array
			to be removed once we can be certain it will have been updated to 1.1+
		*/
		// lets not dp that for now
		//if(defined('WPPIZZA_CRN_CURRENT_VERSION') &&  !empty($order['wppizza_crn'])){
		//	$order = (object) $order;
		//}
...
*
********************************************/
public static function customer_confirmation_formatted($called_by, $get_template_parameters = false){
	global $wppizza_options, $blog_id;
	...
	/* only if enabled and not just getting templaet parameters */
	if(empty($wppizza_options['confirmation_form']['confirmation_form_enabled']) || $get_template_parameters){return;}
...
*
********************************************/
public static function customer_details_formatted($called_by, $order = false, $get_template_parameters = false, $template_key = false, $set_values = false, $order_formatted = false, $session = false){
	global $wppizza_options;
	$txt = $wppizza_options['localization'] + $wppizza_options['confirmation_form']['localization'];
	...
	*/
	if($get_template_parameters){
		...
		/* these should already be saved as sorted - distinctly unsetting tips here */
		$formfields = WPPIZZA()-> helpers -> enabled_formfields(false, false, true, $get_template_parameters);


		/* distinctly unset tips here*/
		//unset($formfields['ctips']);
		...
		foreach($formfields as $ff_key=>$ff_values){
			...
			*************************************************************************/
			if(!empty($set_values)){
				...
				$added_formfields = array_diff_key($formfields, $set_values['sort'][$section_key] );
	...
	*/
	$order['blog_options']['order_form'] = apply_filters('wppizza_filter_formfields', $order['blog_options']['order_form'],  $called_by);
	/* alias filter - going forward the above will - at some point - be deprecated to avoid confusion with other filters*/
	$order['blog_options']['order_form'] = apply_filters('wppizza_register_formfields', $order['blog_options']['order_form'],  $called_by);
	...
	/*
		...
		session setup, db collation, caching issues  (opcache perhaps ?) etc etc . and if not one of these, then i really do not know at the moment
	*/
	...
	foreach($order['blog_options']['order_form'] as $key => $val){
		...
		if( ( empty($session) && $val['type'] !='tips' && !empty($val['enabled']) ) ||  (  !empty($session) && !empty($val['enabled']) )  ){
			...
			/*
				fields returned only when session (i.e for input fields on order page)
			*/
			if(!empty($session)){
				/*********
...
*
********************************************/
	public static function itemised_details_formatted($called_by, $order = false, $get_template_parameters = false, $template_key = false, $set_values = false, $order_formatted = false){
		global $wppizza_options;
		...
		*/
		...
		if($template_key==-1 && !empty($order_formatted)){
			$default_enable['tax_rate_formatted'] = ($order_formatted['sections']['order']['multiple_taxrates']) ? true : false;
		}else{
		...
		*/
		if($get_template_parameters){
			...
			*/
			...
			$template_enabled_parameters['labels']['parameters']['quantity'] 	= $order['blog_options']['localization']['itemised_label_quantity'];
			$template_enabled_parameters['labels']['parameters']['article'] 	= $order['blog_options']['localization']['itemised_label_article'];
			$template_enabled_parameters['labels']['parameters']['taxrate'] 	= $order['blog_options']['localization']['itemised_label_taxrate'];
			$template_enabled_parameters['labels']['parameters']['total'] 		= $order['blog_options']['localization']['itemised_label_total'];
			...
			*************************************************************************/
			if(!empty($set_values)){
				...
				if($get_template_parameters=='emails'){
            		...
            		$template_enabled_parameters['style']['td-ctr'] = !empty($set_values['sections'][$section_key]['style']['td-ctr']) ? $set_values['sections'][$section_key]['style']['td-ctr'] : '';
            		$template_enabled_parameters['style']['td-rgt'] = !empty($set_values['sections'][$section_key]['style']['td-rgt']) ? $set_values['sections'][$section_key]['style']['td-rgt'] : '';
            		$template_enabled_parameters['style']['td-blogname'] = !empty($set_values['sections'][$section_key]['style']['td-blogname']) ? $set_values['sections'][$section_key]['style']['td-blogname'] : '';
            		$template_enabled_parameters['style']['td-catname'] = !empty($set_values['sections'][$section_key]['style']['td-catname']) ? $set_values['sections'][$section_key]['style']['td-catname'] : '';
				}
				...
				foreach($set_values['sort'][$section_key] as $parameter_key=>$enabled){
					...
					/* css*/
					if($get_template_parameters=='emails'){
					$template_enabled_parameters['style']['td-'.$parameter_key] = !empty($set_values['sections'][$section_key]['style'][$parameter_key.'-tdall']) ? $set_values['sections'][$section_key]['style'][$parameter_key.'-tdall'] : '';
					}
...
*
********************************************/
	public static function summary_details_formatted($called_by, $order = false, $get_template_parameters = false, $template_key = false, $set_values = false, $order_formatted = false){
...
//			/*
//				applied tax rates for output formatting
//			*/
//			$taxrates_set = array();
//			if(!empty($order_values['param']['taxrate'])){/*main taxrate*/
//				$taxrates_set[] = wppizza_output_format_percent($order_values['param']['taxrate'], true);
//			}
//			if(!empty($order_values['param']['taxrate_alt'])){/*alt taxrate*/
//				$taxrates_set[] = wppizza_output_format_percent($order_values['param']['taxrate_alt'], true);
//			}
//			$taxrates_sprintf=!empty($taxrates_set) ? implode(' / ', $taxrates_set) : '';
		}
		...
		*/
		if($get_template_parameters){
			...
			*************************************************************************/
			...
			//$template_enabled_parameters = apply_filters('wppizza_filter_summary_details_enabled', $template_enabled_parameters, $order_values, $template_key);
		return $template_enabled_parameters;
		}


		$summary = apply_filters('wppizza_filter_summary_details_formatted', $summary, $order_values, $called_by, $template_key);
