...
class WPPIZZA_MARKUP_PAGES{
	...
	function thankyou_order_info($param, $type = 'thankyoupage'){
		...
		*/
		if(is_object($post) && $wppizza_options['order_settings']['orderpage'] == $post->ID && ( isset($_GET[WPPIZZA_TRANSACTION_GET_PREFIX]) || isset($_GET[WPPIZZA_TRANSACTION_CANCEL_PREFIX]) ) ){
			...
			/*
				get order from transaction prefix hash
			*/
			if(isset($_GET[WPPIZZA_TRANSACTION_GET_PREFIX])){
				$results = WPPIZZA()->db->fetch_order_details(false, false, $_GET[WPPIZZA_TRANSACTION_GET_PREFIX], array('COMPLETED', 'FAILED', 'INPROGRESS', 'AUTHORIZED', 'CAPTURED', 'CANCELLED', 'UNCONFIRMED', 'PAYMENT_PENDING' ), false);
			}
			/*
				get order from cancel prefix hash
			*/
			if(isset($_GET[WPPIZZA_TRANSACTION_CANCEL_PREFIX])){
				$results = WPPIZZA()->db->fetch_order_details(false, false, $_GET[WPPIZZA_TRANSACTION_CANCEL_PREFIX], array('COMPLETED', 'FAILED', 'INPROGRESS', 'AUTHORIZED', 'CAPTURED', 'CANCELLED', 'UNCONFIRMED'  ), false);
			}
			...
			*/
			if($results){
				$results_formatted = WPPIZZA()->order->results_formatted($type, $results['orders'][0]);
				$payment_status = $results_formatted['sections']['ordervars']['payment_status']['value'];
	...
	***************************************/
	function localize_js_orderpage_validation_rules($localize){
		...
		*****************************/
		...
		foreach(WPPIZZA()->helpers->enabled_formfields(false, true, false, 'orderpage') as $key=>$value){
	...
	***************************************/
	function order_page($type, $atts , $txt, $user_session){
		...
		$results = WPPIZZA()->db->fetch_order_details(false, false, $user_session['wppizza_hash'],'INITIALIZED');
		$order_results = WPPIZZA()->order->results_formatted($type, $results['orders'][0], true);
	...
	***************************************/
	function confirmation_page($type, $atts = null, $txt, $user_session){
		...
		/* show formfields from session as customer_ini might not have been set yet */
		//set true as well, to also - again - run a $can_checkout check using $order_formatted['checkout_parameters']['can_checkout']. would have performance implications and i think this is overkill for the moment as you would never get to this page without being able to checkout */
		$order_results = WPPIZZA()->order->results_formatted($type, $results['orders'][0]);
		$order_formatted = $order_results['sections'];/* for consistency with carts etc */
	...
	***************************************/
	function thankyou_page($type, $atts = null, $txt, $user_session){
		...
		*/
		...
		if(!$is_completed && !$is_inprogress && !$is_payment_pending && !$is_cancelled && !$is_unconfirmed ){
			/**get links to order/amend pages*/
		...
		***********************************************************/
		if($is_completed || $is_inprogress || $is_payment_pending || $is_unconfirmed){
	...
	function orderhistory_page($type, $atts = null, $txt, $user_session){
		...
		*/
		if(is_user_logged_in()){
			...
			*/
			...
			$orders_completed = WPPIZZA()->db->get_orders($args);
		...
		*/
		...
		/*
			apply filter if required and implode for output
		*/
		$markup = apply_filters('wppizza_filter_pages_purchasehistory_markup', $markup, $purchase_history);
		$markup = implode('', $markup);
