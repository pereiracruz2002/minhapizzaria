...
*********************************************************************************************/
		...
		*
		*************************************************************/
		function admin_template_get_values($tplKey, $msgKey, $arrayIdent, $set_values){

			/* in case we add some other template types one day */
			if($tplKey=='print' || $tplKey=='emails'){
				/*
					section/template values
				*/
				$tpl_values = WPPIZZA()->order->results_formatted('template_values_'.$tplKey.'', false, false, false, false, $tplKey, $msgKey, $set_values );
			}
		return $tpl_values;
		}
		...
		*************************************************************/
		function admin_template($msgKey, $templateKey, $set_values = false, $arrayIdent = 'templates'){
			...
			*/
			$tplVals = $this->admin_template_get_values($templateKey, $msgKey, $arrayIdent, $set_values);
						...
						$markup.='<tr>';
							...
							**/
							$markup.='<td class="'.WPPIZZA_SLUG.'-'.$arrayIdent.'-main-buttons">';
									/*icon edit*/
									$markup.="<span id='".WPPIZZA_SLUG."_".$arrayIdent."_toggle_".$templateKey."_".$msgKey."' class='".WPPIZZA_SLUG."_".$arrayIdent."_details_toggle  wppizza-dashicons dashicons-edit' title='".__('edit', 'wppizza-admin')."'></span>";

									/*icon code edit (show css/style)*/
									$markup.='<span id="'.WPPIZZA_SLUG.'-dashicons-'.$arrayIdent.'-'.$templateKey.'-media-code-'.$msgKey.'" class="wppizza-dashicons dashicons-media-code '.$tplVals['htmlactiveclass'].'" title="'.$tplVals['htmlactivetitle'].'"></span>';

									/*icon preview*/
									$markup.="<span  id='".WPPIZZA_SLUG."_".$arrayIdent."_".$templateKey."_preview-".$msgKey."' class='".WPPIZZA_SLUG."_".$arrayIdent."_preview wppizza-dashicons dashicons-visibility' title='".__('preview', 'wppizza-admin')."'></span>";
									...
									$markup.="<span id='".WPPIZZA_SLUG."_".$arrayIdent."_".$templateKey."_delete-".$msgKey."' class='".WPPIZZA_SLUG."_".$arrayIdent."_delete wppizza-dashicons dashicons-trash' title='".__('delete', 'wppizza-admin')."'></span>";
									}
							$markup.='</td>';
						$markup.='</tr>';
					$markup.='</tbody>';
...
*
****************************************************************************************************************************************************************************************************************************/
	function get_template_email_plaintext_sections_markup($order_formatted, $localization, $template_parameters, $template_type){
		$tpl = array();
		...
		if(!empty($section_values['section_enabled'])){
			...
			if($section_key == 'site'){
				...
				****************************************/
				if(!empty($section_values['label_enabled'])){
					$tpl[$section_key]['section_label'] = trim($localization['templates_label_'.$section_key.'']);
			...
			**************************************/
			if($section_key == 'order'){
				...
				***************************************/
				$order_columns = WPPIZZA()->helpers->itemised_order_columns($order_formatted, $template_parameters, $template_type, $localization );
		...
		**************************************/
		if($template_type=='emails'){
			...
			if(!empty($localization['templates_footer_note'])){
				$tpl[$section_key]['footer_note'] = apply_filters('wppizza_filter_plaintext_line', $localization['templates_footer_note'], ' ', true);
			}
			/** implode for output */
			$tpl[$section_key] = implode(PHP_EOL,$tpl[$section_key]);
			/* allow filtering of whole section string (maybe using regexes) */
...
*
***********************************************************************************************************************************************************************************************************************************/
	function get_template_email_html_sections_markup($order_formatted, $localization, $template_parameters, $template_type, $template_id){
				...
				if($section_key == 'site'){
					...
					****************************************/
					if(!empty($section_values['label_enabled'])){
						...
						$section_label = trim($localization['templates_label_'.$section_key.'']);
					...
					***************************************/
					foreach($section_values['parameters'] as $value_key => $values){
						...
						if(!empty($values['enabled']) && !empty($order_formatted['sections'][$section_key][$value_key])){
							...
							/***wrap in tr/td**/
							if($template_type=='emails'){
								$tpl[$section_key][$value_key] ='<tr><td style="'.$section_values['style']['td-ctr'].';'.$section_values['style'][$value_key.'-tdall'].'">'.$row['ctr'].'</td></tr>'.PHP_EOL;
							}
					...
					foreach($section_values['parameters'] as $value_key => $values){
						...
						if(!empty($values['enabled']) && !empty($order_formatted['sections'][$section_key][$value_key])){
							...
							if($value_key=='order_date' ||  $value_key=='pickup_delivery' ||  $value_key=='admin_notes'){
								/***wrap in tr/td**/
								if($template_type=='emails'){
									$tpl[$section_key][$value_key] ='<tr><td colspan="2" style="'.$section_values['style'][$value_key.'-tdall'].'">'.$row['rgt'].'</td></tr>'.PHP_EOL;
								}
								if($template_type=='print'){
								...
								/***wrap in tr/td**/
								if($template_type=='emails'){
									$tpl[$section_key][$value_key] ='<tr><td style="'.$section_values['style']['td-lft'].';'.$section_values['style'][$value_key.'-tdall'].'">'.$row['lft'].'</td><td style="'.$section_values['style']['td-rgt'].';'.$section_values['style'][$value_key.'-tdall'].'">'.$row['rgt'].'</td></tr>'.PHP_EOL;
								}
								if($template_type=='print'){
					...
					foreach($section_values['parameters'] as $value_key => $values){
						...
						if(!empty($values['enabled']) && !empty($order_formatted['sections'][$section_key][$value_key])){
							foreach($order_formatted['sections'][$section_key][$value_key] as $vKey => $vals){
								...
								/***wrap in tr/td**/
								if($template_type=='emails'){
									$tpl[$section_key][$value_key.'_'.$vKey] ='<tr><td style="'.$section_values['style']['td-lft'].';'.$section_values['style'][$value_key.'-tdall'].'">'.$row['lft'].'</td><td style="'.$section_values['style']['td-rgt'].';'.$section_values['style'][$value_key.'-tdall'].'">'.$row['rgt'].'</td></tr>'.PHP_EOL;
								}
								if($template_type=='print'){
		...
		***********************************************************************/
		if($template_type=='emails'){
			...
			if(!empty($localization['templates_footer_note'])){
				$tpl[$section_key]['footer_note'] = $localization['templates_footer_note'];
			}
