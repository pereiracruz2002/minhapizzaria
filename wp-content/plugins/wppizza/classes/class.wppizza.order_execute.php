...
class WPPIZZA_ORDER_EXECUTE{
	...
	*/
	function get_initialized_order(){
		...
		****************************************************************/
		$order = WPPIZZA()->db->fetch_order_details(false, false, $order_hash, 'INITIALIZED');
		if(empty($order)){
			/* error details */
			$result['error'][] = array(
				'critical'=> false, /* force sending of email to admin */
		...
		**/
		$order_formatted = WPPIZZA()->order->results_formatted('get_initialized_order', $order['orders'][0], true);
		$order_formatted = WPPIZZA()->order->simplify_order_values($order_formatted, array('localization'));
		...
		***************************************************************/
		...
		if(!$checkout_parameters['can_checkout']){
			...
			$result['error'][] = array(
				'critical'=> false, /* force sending of email to admin */
				'error_id'=> 20002 ,
				'error_message' => __('cannot checkout' ,'wppizza-admin') . PHP_EOL . print_r($checkout_parameters, true) . PHP_EOL . print_r($server_vars, true),
				'wp_error' => ''
			);
			/* logging, and sending */
			$this->gateway_logging($result['error'], false, false, $this->wppizza_gateway_ident, 'prepare-error');
		...
		if(!$checkout_parameters['shop_open']){
			...
			$result['error'][] = array(
				'critical'=> false, /* force sending of email to admin */
				'error_id'=> 20003 ,
				'error_message' => __('shop is closed','wppizza-admin'),
				'wp_error' => ''
			);
			/* logging, and sending */
			$this->gateway_logging($result['error'], false, false, $this->wppizza_gateway_ident, 'prepare-error');
		...
		****************************************************************/
		...
		if(empty($order)){
			...
			**/
			...
			$order_update = WPPIZZA()->db->update_order(false, $order['id'], false , $update_db_values, 'INITIALIZED');
			if(!$order_update){
				/* error details */
				$result['error'][] = array(
				...
				/* logging, and sending */
				$this->gateway_logging($result['error'], false, $order['id'], $this->wppizza_gateway_ident, 'prepare-error');
			...
			*/
			$order = WPPIZZA()->db->fetch_order_details(false, $order['id'], false, 'INPROGRESS');
			if(empty($order)){
				/* error details */
				$result['error'][] = array(
					'critical'=> false, /* force sending of email to admin */
			...
			/**
				...
				order details are always arrays
			**/
			$order_formatted = WPPIZZA()->order->results_formatted('order_prepare', $order['orders'][0], true);
			$order_formatted = WPPIZZA()->order->simplify_order_values($order_formatted, array('localization'));
		}


		/*
			no errors thrown anywhere, return
			order details
		*/
		$result['order'] = $order_formatted;
	...
	*********************************************************************************************************/
	function order_execute($order_details, $tx_id = null, $tx_details = false, $tx_errors = false, $ipn = false, $check_user_id = null, $custom_update_columns = false, $unconfirmed = null, $send_emails = true){
		...
		****************************************************************/
		...
		$update_db_values['payment_status'] 	= ($unconfirmed === null) ? array('type'=> '%s', 'data' => 'CAPTURED') : (($unconfirmed === true) ? array('type'=> '%s', 'data' => 'UNCONFIRMED') : array('type'=> '%s', 'data' => 'CONFIRMED'))  ;
		...
		if(!$order_update_captured){
			...
			$result['error'][] = array(
				'critical'=> true, /* force sending of email to admin */
				'error_id'=> 10001,
				'error_message' => $ipn_error_prefix . __('DB: Could not update order: INITIALIZED|INPROGRESS => CAPTURED|UNCONFIRMED','wppizza-admin'),
				'wp_error' => '',
		...
		***************************************************************/
		/*
			TODO/REPLACE
			currently $order_details does not provide all the details
			to use in get_email_templates(), so lets get this again
			get_email_templates() might need to be fixed for this one day
			(to be determined, might actually need to keeop this so $order_parameters_formatted is consistent)
		*/
		...
		*/
		...
		$order_for_templates = WPPIZZA()->db->fetch_order_details(false, $order_id, false, $captured_status , $check_user_id);
		$order_for_templates = $order_for_templates['orders'][0];
		$order_parameters_formatted = WPPIZZA()->order->results_formatted('order_execute', $order_for_templates);//, false, false


		/****/
		$email_templates = $this->get_email_templates($order_for_templates, $order_parameters_formatted);
		...
		*/
		if($send_emails === true ){
			...
			/****
				if we are running any of the filters below
				get order details formatted (simplyfy them a bit) and pass it along to the filters when sending emails
			*****/
			$email_parameters_formatted = WPPIZZA()->order->simplify_order_values($order_parameters_formatted, array('localization'));
			$email_results = WPPIZZA()->email->send($email_templates, $email_parameters_formatted, $order_for_templates);//, $order_captured_formatted_raw
		...
		***************************************************************/
		$send_emails = apply_filters('wppizza_on_order_execute_send_email', $send_emails, $order_parameters_formatted , $tx_id, $tx_details, $email_templates);
		...
		***************************************************************/
		if(!empty($user_create_account) && empty($wp_user_id)){
			WPPIZZA()->user->create_account($order_id, $order_details['customer']);
		}
		/***************************************************************
		...
		***************************************************************/
		if(!empty($user_update_profile) && !empty($wp_user_id)){
			WPPIZZA()->user->update_profile($wp_user_id, $order_details['customer']);
		}
		...
		**************************************************************/
		if(has_action('wppizza_on_order_execute') && $completed_status == 'COMPLETED') {
			...
			/*
				get full order details, including transaction values etc now
				as we are running an add_action on this hook
			*/
			$details_completed = WPPIZZA()->db->fetch_order_details(false, $order_id, false, 'COMPLETED', false);
			$details_completed = $details_completed['orders'][0];
			$details_completed = WPPIZZA()->order->results_formatted('get_completed', $details_completed);			
			$details_completed = WPPIZZA()->order->simplify_order_values($details_completed, array('localization'));


			/* hook into this */
			do_action('wppizza_on_order_execute', $order_id, $details_completed, $print_templates);
		}
		/***********************************************************
	...
	*******************************************************************************************************************/
	function get_prepared_order($order_id = false, $gateway_reply = false, $critical = false, $wp_user_id = null){
		...
		****************************************************************/
		$order = (!empty($order_id)) ? WPPIZZA()->db->fetch_order_details(false, $order_id, false, array('INPROGRESS','UNCONFIRMED'), $wp_user_id) : WPPIZZA()->db->fetch_order_details(false, false, $order_hash, array('INPROGRESS','UNCONFIRMED'), $wp_user_id) ;
		if(empty($order)){
			...
			**/
			$order = $order['orders'][0];
			$result = WPPIZZA()->order->results_formatted('get_prepared', $order, true);
	...
	*******************************************************************************************************************/
	function get_completed_order($order_id = false, $gateway_reply = false, $critical = false, $wp_user_id = null , $payment_status = array('COMPLETED')){
		...
		****************************************************************/
		$order =  WPPIZZA()->db->fetch_order_details(false, $order_id, false, $payment_status, $wp_user_id) ;
		if(empty($order)){
			/* error details */
			$result['error'][] = array(
				'critical'=> $critical, /* force sending of email to admin */
			...
			**/
			$order = $order['orders'][0];			
			$result = WPPIZZA()->order->results_formatted('get_completed', $order);
			$result = WPPIZZA()->order->simplify_order_values($result, array('localization'));
		}
	...
	function get_order_by_columns($columns = false, $simplify = true){
		...
		***************************************************************/
		...
		if (!empty($order)) {
			...
			$result = WPPIZZA() -> order -> results_formatted('get_order_by_columns', $order);
			if(!empty($simplify)){
				$result = WPPIZZA()->order->simplify_order_values($result, array('localization'));
			}
	...
	***************************************************************/
	function get_email_templates($order, $order_parameters_formatted, $template_type = 'emails'){
		global $wppizza_options;
		...
		*/
		...
		foreach($enabled_formfields as $cdataKey=>$cDataVal){
			if(!empty($cDataVal['add_to_subject_line'])){
				...
				$set_user_value = trim($order['customer_ini'][$cdataKey]);
		...
		$order['blog_info'] = apply_filters('wppizza_email_subject_bloginfo', $order['blog_info']);

		/** create 2 versions of bloginfo for standard output or output with customer details in subject */
		$blog_info = array();
		$blog_info['standard'] 	= $order['blog_info']['blogname'];
		$blog_info['custom'] 	= empty($order['blog_info']['blogname']) ?  '' : '['.$order['blog_info']['blogname'].'] - ';
		...
		$subject['main'] 	= $order['blog_options']['localization']['your_order'];
		$subject['suffix'] 	= date_i18n($order['date_format']['date'], WPPIZZA_WP_TIME)." ".date_i18n($order['date_format']['time'], WPPIZZA_WP_TIME)."";


		/** filter, implode, decode */
		$subject = apply_filters('wppizza_filter_email_subject', $subject, $order, $order_parameters_formatted['sections']);
		...
		***************/
		$customer = false;
		$reply_to = false;
		if(!empty($order['customer_ini']['cemail'])){

			/* let's try and make a name from customer email */
			$cname = explode('@',$order['customer_ini']['cemail']);
			$cname=str_replace(array('.'),' ',$cname[0]);/*worth a go*/
			/* add as zero index for consistency */
			$customer[0] = array('name' => $cname, 'email' => $order['customer_ini']['cemail']);
			/* set reply to as weel to be customer */
			$reply_to = array('name' => $cname, 'email' => $order['customer_ini']['cemail']);
		}
		...
		************************************************************************************************/
		...
		foreach($used_template_ids as $template_id){
			...
			**/
			$template_settings[$template_id]['plaintext'] =  WPPIZZA()->templates_email_print->get_template_email_plaintext_sections_markup($order_parameters_formatted, $order['blog_options']['localization'], $template_values, 'emails');

			/*
				get html markup (if required)
			*/
			$template_settings[$template_id]['html'] = ($is_html) ?  WPPIZZA()->templates_email_print->get_template_email_html_sections_markup($order_parameters_formatted, $order['blog_options']['localization'], $template_values, 'emails', $template_id) : false;
		}
	...
	***************************************************************/
	function get_print_templates($tpl_ids, $order_id, $localization, $order_formatted, $template_type = 'print'){
		$template_data = false;
		...
		$get_templates = apply_filters('wppizza_filter_template_options', $get_templates, $template_type);
			foreach($tpl_ids as $tpl_id){
				if(!empty($get_templates[$tpl_id])){
					...
					****************************************/
					if($as_html){
						$markup = WPPIZZA()->templates_email_print->get_template_email_html_sections_markup($order_formatted, $localization, $template_values, $template_type, $tpl_id);
						...
						$parameters = WPPIZZA()->templates_email_print->get_template_email_plaintext_sections_markup($order_formatted, $localization, $template_values, $template_type);
