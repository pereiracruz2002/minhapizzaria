...
class WPPIZZA_MODULE_SETTINGS_MULTISITE{
	...
	function __construct() {
		...
		***********************************************************/
		if(is_multisite()){
			...
			add_filter('wppizza_filter_site_details_formatted', array( $this, 'site_details_formatted'), 10, 3 );
	...
	*/
	function site_details_formatted($site_parameters , $order, $get_template_parameters){
		
		/* skip if there's nothing to do in the first place */
		if(empty($order['blog_info']) &&  !$get_template_parameters){
		...
		*/				
		if(!$get_template_parameters && $order['blog_info']['blog_id'] != BLOG_ID_CURRENT_SITE){
			$has_parent_blog = true;
			$parent_blog = (array) get_blog_details(BLOG_ID_CURRENT_SITE); 
		}
						
		if(!empty($has_parent_blog) || $get_template_parameters){	
