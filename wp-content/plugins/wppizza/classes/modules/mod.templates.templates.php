		...
		if($_POST['vars']['field']=='preview_template'){
			...
			****************************************/
			...
			/****************************************
				...
				as preview
			****************************************/
			if(!is_array($order)){// || $lastOrder->id <= 0
			...
			*****************************************/
			if(empty($sections_enabled)){
				...
				/* format parameters from order -> formatted */
				$order_parameters_formatted = WPPIZZA()->order->results_formatted('preview_template_'.$preview_type.'', $order);/* format parameters from order */

				/** get plaintext  */
				if(!$preview_is_html){
					$plaintext_data =  WPPIZZA()->templates_email_print->get_template_email_plaintext_sections_markup($order_parameters_formatted, $order['blog_options']['localization'], $template_parameters, $preview_type);
					$preview['markup']['plaintext'] = $plaintext_data['markup'];
				}

				/** get html output */
				if($preview_is_html){
					$html_data =  WPPIZZA()->templates_email_print->get_template_email_html_sections_markup($order_parameters_formatted, $order['blog_options']['localization'], $template_parameters, $preview_type, $preview_id);
