	...
	function admin_ajax($wppizza_options){
		...
		***********************************************************************************/
		if($_POST['vars']['field']=='new_orders'){
			...
			*/
			...
			$new_orders = WPPIZZA() -> db -> get_orders($args);
		...
		********************************************/
		if($_POST['vars']['field']=='refund_at_gateway'){
			...
			*/
			$get_order = WPPIZZA() -> db ->fetch_order_details($blog_id, $order_id, false, array('COMPLETED', 'REFUNDED'), false);
			if(empty($get_order)){
			...
			*/
			$order_results = WPPIZZA()->order->results_formatted('refund_gateway', $get_order['orders'][0], true);
		...
		*************************************************************************************/
		if($_POST['vars']['field']=='print-order' && $_POST['vars']['id']>=0){
			...
			/****************************************
				get the order
			****************************************/
			$order = WPPIZZA()->db->fetch_order_details($blog_id, $order_id, false, array('COMPLETED','REFUNDED','REJECTED'), false);
			...
			****************************************/
			...
			/****************************************
				format parameters from order details
			****************************************/
			$order_parameters_formatted = WPPIZZA()->order->results_formatted('print_order', $order);
			...
			****************************************/
			if($as_html){
				$obj['markup']['html'] = WPPIZZA()->templates_email_print->get_template_email_html_sections_markup($order_parameters_formatted, $order['blog_options']['localization'], $template_values, $template_type, $template_id );

			}else{
				/* plaintext returns sections too, so get the array first */
				$tpl = WPPIZZA()->templates_email_print->get_template_email_plaintext_sections_markup($order_parameters_formatted, $order['blog_options']['localization'], $template_values, $template_type );
						...
						$order_markup['summary_tr_'] = "<tr id='".WPPIZZA_SLUG."-".$this->section_key."-ordersummary-".$uoKey."' class='".WPPIZZA_SLUG."-".$this->section_key."-ordersummary ".WPPIZZA_SLUG."-".$this->section_key."-orderstatus-".$order_status_class."".$payment_unconfirmed_class." ".$summary_visibility_class."'>";
							...
							****************************************************************/
							$order_markup['summary_actions_td_']="<td id='".WPPIZZA_SLUG."-".$this->section_key."-column-actions-summary-".$uoKey."' class='".WPPIZZA_SLUG."-".$this->section_key."-column-actions-summary'>";
								...
								*******************************/
								if (current_user_can('wppizza_cap_delete_order')){
									$ordersummary_actions['bulk_delete'] = '<span class="'.WPPIZZA_SLUG.'_'.$this->section_key.'_delete-select"><input class="'.WPPIZZA_SLUG.'_'.$this->section_key.'_delete-selected" type="checkbox" value="'.$uoKey.'" /></span>';
								}
