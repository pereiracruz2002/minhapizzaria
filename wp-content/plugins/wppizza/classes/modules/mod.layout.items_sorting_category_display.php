...
class WPPIZZA_MODULE_ITEMS_SORTING_CATEGORY_DISPLAY{
	...
	function __construct() {
		...
		***/
		...
		add_filter('wppizza_filter_order_items_markup', array( $this, 'get_category_for_cart_email_order'));
		add_filter('wppizza_filter_email_items_markup', array( $this, 'get_category_for_cart_email_order'));
		...
		add_filter('wppizza_filter_order_item_markup', array( $this, 'show_category_in_order'), 10, 5 );
	...
	*/
	function show_category_in_plaintext_templates($markup_item, $key, $item){
		...
		if(isset($this -> items_keys_categories[$key]) && $this -> items_keys_categories[$key]['category_path'] !=''){
			$prepend_category_name = '['.$this -> items_keys_categories[$key]['category_path'].']' ;
	...
	*/
	function show_category_in_html_templates($markup_item, $key, $item, $items, $colspan){
		...
		if(isset($this -> items_keys_categories[$key]) && $this -> items_keys_categories[$key]['category_path'] !=''){
			...
			$prepend_category_name = '<tr class="'.WPPIZZA_SLUG.'-item-category"><td colspan="'.$colspan.'" style="'.$style.'">'.$this -> items_keys_categories[$key]['category_path'].'</td></tr>';
			array_unshift($markup_item, $prepend_category_name);
		}
	return $markup_item;
	}
	/*
		add to order markup (checkout page, confirmation page, thank you page, users order history)
	*/
	function show_category_in_order($markup_item, $key, $item, $cart, $colspan ){
		global $wppizza_options;
		/* skip if not enabled */
		if(empty($wppizza_options[$this->layout_page]['items_group_sort_print_by_category'])){
			return $markup_item;
		...
		/* existig colspans plus one for self */
		//$colspan = ($colspan+1);

		/* prepend to array as required */
		if(isset($this -> items_keys_categories[$key]) && $this -> items_keys_categories[$key]['category_path'] !=''){
			$prepend_category_name = '<tr><td class="'.WPPIZZA_SLUG.'-item-category" colspan="'.$colspan.'">'.$this -> items_keys_categories[$key]['category_path'].'</td></tr>';
			array_unshift($markup_item, $prepend_category_name);
		}
	return $markup_item;
	}
	...
	*	only needs to run the first time it's called from somwhere
	**********************************************************************/
	function get_category_for_cart_email_order($items){
		...
		foreach($items as $key => $item){
			...
			if(!isset($do_cat_name[$item_cat_id])){
				$do_cat_name[$item_cat_id] = true;
				...
				*/
				...
				$add_cat_name[$key] = array('category_id' => $item_cat_id, 'category_path' => $category_path, 'category_title' => $category_title  );
			}
		}
		/* all menu item keys as keys that need category name displayed */
		$this -> items_keys_categories = $add_cat_name;
