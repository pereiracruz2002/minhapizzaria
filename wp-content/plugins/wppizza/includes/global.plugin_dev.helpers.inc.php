...
*******************************************************************************/
function wppizza_phi_validate($plugin_slug, $plugin_options, $plugin_settings, $settings_page){//, $settings_page
	...
	*/
	...
	foreach($plugin_settings['fields'][$plugin_slug] as $field_key => $field_parameters){
		...
		*/
		if(!isset($field_parameters['options']) && isset($_POST[$plugin_slug][$settings_page][$field_key])){
			/*
				raw post val
			*/
			$validated = $_POST[$plugin_slug][$settings_page][$field_key] ;
			/*
		...
		*/
		if(isset($field_parameters['options']) && is_array($field_parameters['options'])){
			...
			foreach($field_parameters['options'] as $option_key => $option_parameters){
				...
				*/
				$iteration ++;
				if(isset($_POST[$plugin_slug][$settings_page][$field_key]) && $iteration == 1){
				...
				*/
				...
				foreach($option_parameters['options'] as $sub_option_key => $sub_option_parameters){
					...
					*/
					if(isset($_POST[$plugin_slug][$settings_page][$sub_option_parameters['option']][$sub_option_key])){

						/*
							raw post val
						*/
						$validated = $_POST[$plugin_slug][$settings_page][$sub_option_parameters['option']][$sub_option_key] ;
						/*
							using callback if exists
						*/
		...
		if(isset($_POST[$option_name])){
			...
			*/
			...
			foreach($set_tab_options as $tab_key=>$tab_options){
					...
					/* using callback if exists */
					$validated = wppizza_phs_validate_by_callback($validated, $tab_options['validation_callback']);
					/* option update */
...
function wppizza_phs_validate_by_callback($input, $callback){
	/* no validation set */
	if(empty($callback)){return $input;}
...
function wppizza_phs_section_options($args, $echo = true){
	...
	*/
	if($type == 'email' || $type == 'text'){
		...
		/* if width is set */
		$px_width = !empty($args['width']) ? (int)$args['width'] : 500 ;

		$formfield .= '<label id="'.$slug  .'_'. $tab .'_'. $section .'_'. $field .'">';
		$formfield .= '<input name="'.$slug.'['.$tab.']['.$section.']['.$field.']"  type="text" value="' . stripslashes(esc_html($value)) . '" placeholder="' .stripslashes(esc_html($placeholder)) . '" style="width:'.$px_width.'px" />';
			...
			$formfield .= '<div id="'.$_PLUGIN.'_'. $_PAGE .'_'. $field .'">';
				...
				if(is_array($options)){
					foreach($options as $option_value => $option_array){
						...
						*/
						$formfield .= '<label><input name="'.$_PLUGIN.'['.$_PAGE.']['.$field.']['.esc_html($option_value).']"  type="'.$type.'" value="'.esc_html($option_value).'" '.checked( isset($value[$option_value]) , true, false).'/>'.$option_array['label'].''.$add_br.'</label>';
					...
					$formfield .= '<label><input name="'.$_PLUGIN.'['.$_PAGE.']['.$field.']"  type="'.$type.'" value="'.esc_html($field).'" '.checked($value, true, false).' /></label>';
				}
				if(!empty($info) || !empty($description) ){
					$formfield .= '<span style="margin-left:10px">';
					$formfield .= '' . $info . '' . $description . '';
