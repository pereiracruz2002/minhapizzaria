...
*
*********************************************************/
function wppizza_get_order_email_templates($blog_id = false, $order_id, $order_formatted, $hash = false, $payment_status = false){
	/* fetch order details to construct template outputs */
	$order_for_templates = WPPIZZA()->db->fetch_order_details($blog_id, $order_id, $hash, $payment_status);
	$order_for_templates = $order_for_templates['orders'][0];

	/* get email templates outputs */
	$ORDER_EXECUTE = new WPPIZZA_ORDER_EXECUTE();
	$email_templates = $ORDER_EXECUTE -> get_email_templates($order_for_templates, $order_formatted);
