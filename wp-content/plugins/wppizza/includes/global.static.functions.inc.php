...
/*********************************************************
	[check if we are on orderpage - must be called
	...
	if($is_orderhistory === null && is_object($post)){
		...
		if( has_shortcode( $post->post_content, 'wppizza' ) ) {
			...
			if(
				...
				&& strpos($matches[3][0], 'orderhistory') !== false
			){
...
function wppizza_get_cart($is_checkout = null, $recalculate = false){
	$cart_contents = WPPIZZA() -> session -> get_cart($is_checkout = null, $recalculate = false);
...
function wppizza_cart_summary($is_checkout = null, $recalculate = false){
	$cart_summary = WPPIZZA() -> session -> get_cart_summary($is_checkout = null, $recalculate = false);
return $cart_summary;
}
...
function wppizza_is_current_businessday($timestamp, $timestampcurrent = false){
	...
	if($timestamp<$currentbusinessday['open'] || $timestamp>$currentbusinessday['close']){
