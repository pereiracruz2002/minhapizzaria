...
if(isset($_POST['vars']['type']) && $_POST['vars']['type']=='reorder' && !empty($_POST['vars']['id'])){
	...
	*********/
	$order = WPPIZZA()->db->fetch_order_details($selected_blog_id, $selected_order_id, false, 'COMPLETED');
	if(!empty($order)){
	...
	if(isset($_POST['vars']['type']) && $_POST['vars']['type']=='admin-order-history'){
		...
		*/
		if(!empty($atts['audio_notify'])){
			...
			$new_orders = wppizza_get_orders($args);
	...
	if( isset($_POST['vars']['type']) && $_POST['vars']['type']=='admin-view-order' && !empty($_POST['vars']['uoKey']) ){
			...
			*/
			...
			/****************************************
				get the order
			****************************************/
			$order = WPPIZZA()->db->fetch_order_details($blog_id, $order_id, false, array('COMPLETED','REFUNDED'), false);
